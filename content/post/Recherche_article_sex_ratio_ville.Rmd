...
# Recherche des villes de plus de 100 000 habitants sur le site de l'INSEE  
...
## Pb avec les villes qui ont des arrondissements  
...
BanR n'est pas adapté car le noms des rues et tous sont en majuscule, la base prends une place folle.  
En fait, peut-être que si...  
  
```{r recherche_banr}

communes_de_plus_de_100000_habitants %>% 
  distinct(DEPCOM) %>% 
  arrange(DEPCOM)
...
ban_06 <- read.csv2(
  "content/post/ban/adresses-06.csv",
  encoding = "UTF-8"
)
...
noms_de_rues_nice <- ban_06 %>%
  filter(
    id_fantoir %>% 
      str_detect("06088") == TRUE
  ) %>%
  distinct(nom_voie)

# on retire les noms qui ne nous intéresse pas, c'est-à-dire, Rue du Lavoir, Avenue du Beau Site....
# il nous faut ceux qui n'ont que des mots commençant par des majuscules (pb : Allfred de Musset, Prince de Galles...)
...
```{r recherche_nom_rues_a_la_main}
