---
title: Text-mining pour créer un nuage de mots représentatif à partir d'un PDF
...
description : "Text-mining avec le package {tidytext}, nuage de mots avec le package {wordcloud} et récupération d'une liste de mots sur internet avec le package {rvest}" 
output:
  blogdown::html_page:
    toc: true
---
...
Pour pouvoir travailler sur la liste de mots, il faut une liste des mots à retirer. Pour la langue anglaise, cette liste est directement disponible dans le package {tidytext} mais il n'y en a pas (à ma connaissance) en français donc j'ai été la récupérer sur le [site count words](https://countwordsfree.com/stopwords/french). Ce site a l'avantage de proposer pleins de langues !    
...
## Mesure des répétitions des mots seuls  
...
# création de la liste de mots et graphique associé
texte <- curriculum %>% 
  mutate(texte = texte %>% 
           str_remove_all("\\d") %>%
...
Si vous regardez attentitivement le code vous verrez que j'ai fait de la lemnisation, c'est-à-dire que j'ai chercher à exploiter la signification des mots et non le mot en tant que telle.  
...
Un mot seul ne signifie pas toujours quelque chose, d'où a necessité d'exploiter aussi les n-grammmes (ici je m'arreterais au bigramme, mais il est bien sûr possible de chercher le nombre de mots que l'on veut !).  
  
```{r creation_du_fichier_bigram, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}

# Creation du fichier bigram pour les expressions et représentation graphique
bigram <- curriculum %>% 
  mutate(texte = texte %>% 
           str_remove_all("\\d") %>%
           str_replace_all("[:punct:]", " ") %>% 
...
# Attention, il faut retirer des mots uniques les mots déjà présents dans le bigram
