...
creation_du_support <- function(num_identifiant_pingouin){
  read_pptx(emplacement_du_support) %>% 
    ...
    ph_with(
      value = "Information sur Marie Vaugoyeau",
...
map2(
  ...
  .f = ~ sendmail(
    from = glue("<{.x}>"),
    to = glue("<{.y}>"),
    subject = glue("Synthèse mensuelle du pingouin {num_identifiant_pingouin}"),
    msg = list(
...
Mais pourquoi ??? ;-(  
En fait, un petit coup de fil rapide à [David Gohel](https://twitter.com/davidgohel), 1 minute de conversation et hop la solution apparaît comme une évidence, ce que je traite comme un pptx est en fait un rpptx... Il faut donc éditer le support pptx avant de l'envoyer et là ça passe sans soucis !  
  
Deux solutions, soit on enregistre tous les supports pptx que l'on envoie pour en archive (solution prise ici), soit on créé un support temporaire qui sera écrasé.  
   
```{r edition_du_support}

enregistrement_du_support_pptx <- function(num_identifiant_pingouin){
  num_identifiant_pingouin %>% 
    creation_du_support() %>% 
    print(glue('presentation_{num_identifiant_pingouin %>% str_replace_all(".", ",")}.pptx'))
}
...
J'espère que cet article vous permettra de mieux chercher de l'aide et de réaliser plus facilement vos travaux.  
