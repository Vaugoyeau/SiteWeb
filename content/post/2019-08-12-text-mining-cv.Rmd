...
description: "Text-mining with the package {tidytext}, word cloud with the package {wordcloud} and retrieving a list of words on the internet with the package {rvest}" 
output:
  blogdown::html_page:
    toc: true
---
...
And I admit, especially, I wanted to use the packages {rvest}, {tidytext} and {wordcloud} so I killed two birds with one stone!   
...
## Retrieving a list of words in French  
  
To be able to work on the word list, you need a list of words to remove. For the English language, this list is directly available in the {tidytext} package but there is none (to my knowledge) in French so I went to get it from the [count words site](https://countwordsfree.com/stopwords/french). This site has the advantage of offering a lot of languages!    
...
## Measurement of word repetitions alone  
...
# creation of the word list and associated graphic
texte <- curriculum %>% 
  mutate(texte = texte %>% 
           str_remove_all("\\d") %>%
...
If you look carefully at the code you will see that I have made compensation, that is, I have sought to exploit the meaning of the words and not the word as such.  
...
# Creation of the bigram file for expressions and graphic representation
bigram <- curriculum %>% 
  mutate(texte = texte %>% 
           str_remove_all("\\d") %>%
           str_replace_all("[:punct:]", " ") %>% 
