...
Before to start anything, I test the data to find outliers (observation that has a
relatively large or small value compared to the majority of observations [Zuur et al. (2010)](http://onlinelibrary.wiley.com/doi/10.1111/j.2041-210X.2009.00001.x/abstract)).  
...
{
boxplot(X[,i]~X$Date, xlab= "Date", asp= 1.5, ylab= variable.names(X)[i])
}
```
...
for (i in c(10,14,18,24))
{
p<-xyplot(Z[,i] ~ Z$Date, groups=Z$Site, ylab= variable.names(Z)[i], xlab="Date", auto.key=list(columns=4))
print(p)
}
```
...
If you have any comments, do not hesitate to [contact me](https://mvaugoyeau.netlify.com/aboutme/) in English or in French.  